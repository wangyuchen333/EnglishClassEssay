...
namespace VCX::Labs::Animation {
    ...
    void CaseMassSpring::OnSetupPropsUI() {
        if (ImGui::CollapsingHeader("Algorithm", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            ImGui::SliderFloat("Spr. Alpha.", &_massSpringSystem.Alpha, .99f, 1.f);
            ImGui::SliderFloat("Gravity", &_massSpringSystem.Gravity, .1f, 1.f);
            ImGui::SliderInt("Iteration", &_massSpringSystem.Iteration, 25, 100);
        }
        ImGui::Spacing();
    ...
    Common::CaseRenderResult CaseMassSpring::OnRender(std::pair<std::uint32_t, std::uint32_t> const desiredSize) {
        if (! _stopped) AdvanceMassSpringSystem(_massSpringSystem, Engine::GetDeltaTime(), _massSpringSystem.prevState, _massSpringSystem.Iteration);
    ...
    void CaseMassSpring::ResetSystem() {
        ...
        _massSpringSystem.prevState = Eigen::VectorXf::Zero(3 * _massSpringSystem.Positions.size());
        for (int i = 0; i < _massSpringSystem.Positions.size(); i++) {
            _massSpringSystem.prevState[3 * i]     = _massSpringSystem.Positions[i].x;
            _massSpringSystem.prevState[3 * i + 1] = _massSpringSystem.Positions[i].y;
            _massSpringSystem.prevState[3 * i + 2] = _massSpringSystem.Positions[i].z;
        }
    }
} // namespace VCX::Labs::Animation
